...
export function FloorSlab({ floor, units, loading, editable, index = 0, onRoomClick, onAddRoom, onEditFloor, onDeleteFloor }: FloorSlabProps) {
  ...
  return (
    ...
    >
      ...
      <div className={cn('flex flex-wrap gap-2.5 p-3.5', total === 0 && !loading && 'items-center')}>
        <AnimatePresence initial={false}>
          {loading
            ? Array.from({ length: 4 }).map((_, i) => <RoomTileSkeleton key={i} />)
            : units.map((unit, i) => <RoomTile key={unit._id} unit={unit} index={i} onClick={() => onRoomClick?.(unit)} />)}
        </AnimatePresence>
