...
import { LayoutDashboard, Users2, Building2, CreditCard, History, Settings, ChevronsLeft, ShieldCheck } from 'lucide-react';
...
export function SuperAdminSidebar({ mobile, onNavigate }: { mobile?: boolean; onNavigate?: () => void }) {
  const collapsed = useAppSelector((s) => s.ui.sidebarCollapsed) && !mobile;
  const dispatch = useAppDispatch();

  return (
    <div className={cn('flex h-full flex-col bg-ink text-white/90', !mobile && 'border-r border-white/10')}>
      <div className={cn('flex items-center gap-2 px-4 py-5', collapsed && 'justify-center px-2')}>
        ...
        {!collapsed && (
          <div className="leading-tight">
            <p className="font-display text-[15px] font-semibold tracking-tight text-white">Brift</p>
            <p className="text-[10.5px] font-medium uppercase tracking-wider text-white/40">Super Admin</p>
          </div>
        )}
      </div>
        ...
        {navItems.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <motion.div layoutId="sa-nav-pill" className="absolute inset-0 rounded-xl bg-crimson-600" transition={{ type: 'spring', stiffness: 400, damping: 32 }} />
                )}
