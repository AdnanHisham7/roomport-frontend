...
import {
  bookDemoSchema,
  createBuilderSubscriptionSchema,
...
export const createSubscriptionRouter = (controller: SubscriptionController): Router => {
  ...
  router.post('/demo', validate(bookDemoSchema), controller.bookDemo);
  ...
  router.post('/upgrade-request', authorize(UserRole.ADMIN, UserRole.SUPER_ADMIN), validate(requestUpgradeSchema), controller.requestUpgrade);

  // Super admin routes
  router.post('/admin/create',               authorize(UserRole.SUPER_ADMIN), validate(createBuilderSubscriptionSchema), controller.createBuilderSubscription);
