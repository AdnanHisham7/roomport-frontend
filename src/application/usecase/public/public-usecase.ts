...
function buildingCard(b: IBuilding, units: IUnit[]): PublicBuildingCardDTO {
  ...
  return {
    ...(b as any), _id: b._id!, slug: b.slug,
...
export class PublicUseCases implements IPublicUseCases {
  ...
  async listBuildings(filter: PublicBuildingListFilter, page: number, limit: number): Promise<PaginatedResult<PublicBuildingCardDTO>> {
    const candidates = await this.buildingRepo.findAll({ isPublished: true, status: 'active', city: filter.city, state: filter.state, type: filter.type as any, search: filter.search });
    const units = await this.unitRepo.findByBuildingIds(candidates.map(b => b._id!));
    const unitsByBuilding = new Map<string, IUnit[]>();
    for (const u of units) { const arr = unitsByBuilding.get(u.buildingId) ?? []; arr.push(u); unitsByBuilding.set(u.buildingId, arr); }
    ...
    if (filter.bedrooms !== undefined) cards = cards.filter(c => (unitsByBuilding.get(c._id) ?? []).some(u => u.status === 'available' && u.bedrooms === filter.bedrooms));
    ...
    return { data: cards.slice((page - 1) * limit, page * limit), total, page, limit, totalPages: Math.ceil(total / limit) || 1 };
  }
  ...
  async getBuildingDetail(idOrSlug: string): Promise<PublicBuildingDetailDTO> {
    ...
    this.buildingRepo.incrementFields(building._id!, { viewCount: 1 }).catch(() => {});
    ...
    const floorsWithStats = floors.sort((a, b) => a.floorNumber - b.floorNumber).map(f => {
      const floorUnits = units.filter(u => u.floorNumber === f.floorNumber.toString());
      return { _id: f._id!, buildingId: f.buildingId, floorNumber: f.floorNumber, name: f.name, totalUnits: f.totalUnits, status: f.status, description: f.description, createdAt: f.createdAt, updatedAt: f.updatedAt, availableUnitsCount: floorUnits.filter(u => u.status === 'available' && !u.isOccupied).length, occupiedUnitsCount: floorUnits.filter(u => u.isOccupied || u.status === 'occupied').length };
    });
